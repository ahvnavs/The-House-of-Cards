...
resource "aws_route_table" "main_route_table" {
    ...
    # Notice: No equals sign. This is a block.
    route {
...
resource "aws_security_group" "east_sg" {
    ...
    ingress {
        ...
        from_port   = tonumber(var.ingress_rule["from_port"])
        to_port     = tonumber(var.ingress_rule["to_port"])
        protocol    = var.ingress_rule["protocol"]
        cidr_blocks = [var.ingress_rule["cidr_blocks"]] # Wrapped in a list
    }

    egress {
        description = var.egress_rule["description"]
        from_port   = tonumber(var.egress_rule["from_port"])
        to_port     = tonumber(var.egress_rule["to_port"])
...
resource "aws_security_group" "east_db_sg" {
    ...
    ingress {
        ...
        to_port         = 5432
        protocol        = "tcp"
        security_groups = [aws_security_group.east_sg.id] # Corrected argument name
    }
...
# Fixed naming collision: "west_sg"
...
# Fixed naming collision: "west_db_sg"
resource "aws_security_group" "west_db_sg" {
    ...
    ingress {
        ...
        to_port         = 5432
        protocol        = "tcp"
        security_groups = [aws_security_group.west_sg.id]
    }
